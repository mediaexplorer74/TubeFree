...
// Type: AngleSharp.Css.ValueConverters.OrderedOptionsConverter
...
namespace AngleSharp.Css.ValueConverters
{
  internal sealed class OrderedOptionsConverter : IValueConverter
  {
    ...
    public OrderedOptionsConverter(params IValueConverter[] converters) => this._converters = converters;
    ...
    public IPropertyValue Convert(IEnumerable<CssToken> value)
    {
      ...
      {
        options[index] = this._converters[index].VaryStart(list);
        if (options[index] == null)
          return (IPropertyValue) null;
      }
      return list.Count != 0 ? (IPropertyValue) null : (IPropertyValue) new OrderedOptionsConverter.OptionsValue(options, value);
    }

    public IPropertyValue Construct(CssProperty[] properties)
    {
      IPropertyValue propertyValue1 = properties.Guard<OrderedOptionsConverter.OptionsValue>();
      if (propertyValue1 == null)
      {
        ...
        propertyValue1 = (IPropertyValue) new OrderedOptionsConverter.OptionsValue(options, Enumerable.Empty<CssToken>());
      }
    ...
    private sealed class OptionsValue : IPropertyValue, IEnumerable<IPropertyValue>, IEnumerable
    {
      ...
      {
        IPropertyValue[] propertyValueArray = this._options;
        for (int index = 0; index < propertyValueArray.Length; ++index)
          yield return propertyValueArray[index];
        propertyValueArray = (IPropertyValue[]) null;
      }

      IEnumerator IEnumerable.GetEnumerator() => this._options.GetEnumerator();
    }
  }
}
