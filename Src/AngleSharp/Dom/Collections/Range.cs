...
// Type: AngleSharp.Dom.Collections.Range
...
namespace AngleSharp.Dom.Collections
{
  internal sealed class Range : IRange
  {
    ...
    public IEnumerable<INode> Nodes => this.CommonAncestor.GetElements<INode>(predicate: new Predicate<INode>(this.Intersects));
    ...
    public IDocumentFragment ExtractContent()
    {
      ...
      INode node1 = !start1.Node.IsInclusiveAncestorOf(end1.Node) ? parent1.GetElements<INode>(predicate: new Predicate<INode>(this.IsPartiallyContained)).FirstOrDefault<INode>() : (INode) null;
      INode node2 = !end1.Node.IsInclusiveAncestorOf(start1.Node) ? parent1.GetElements<INode>(predicate: new Predicate<INode>(this.IsPartiallyContained)).LastOrDefault<INode>() : (INode) null;
      List<INode> list = parent1.GetElements<INode>(predicate: new Predicate<INode>(this.Intersects)).ToList<INode>();
    ...
    public IDocumentFragment CopyContent()
    {
      ...
      INode node1 = !start.Node.IsInclusiveAncestorOf(end.Node) ? parent.GetElements<INode>(predicate: new Predicate<INode>(this.IsPartiallyContained)).FirstOrDefault<INode>() : (INode) null;
      INode node2 = !end.Node.IsInclusiveAncestorOf(start.Node) ? parent.GetElements<INode>(predicate: new Predicate<INode>(this.IsPartiallyContained)).LastOrDefault<INode>() : (INode) null;
      List<INode> list = parent.GetElements<INode>(predicate: new Predicate<INode>(this.Intersects)).ToList<INode>();
      if (list.OfType<IDocumentType>().Any<IDocumentType>())
        throw new DomException(DomError.HierarchyRequest);
      if (node1 is ICharacterData)
      {
    ...
    private bool IsStartBefore(INode node, int offset) => this._start < new Range.Boundary()
    {
      Node = node,
      Offset = offset
    };

    private bool IsStartAfter(INode node, int offset) => this._start > new Range.Boundary()
    {
      Node = node,
      Offset = offset
    };

    private bool IsEndBefore(INode node, int offset) => this._end < new Range.Boundary()
    {
      Node = node,
      Offset = offset
    };

    private bool IsEndAfter(INode node, int offset) => this._end > new Range.Boundary()
    {
      Node = node,
      Offset = offset
    };
    ...
    private struct Boundary : IEquatable<Range.Boundary>
    {
      ...
      public bool Equals(Range.Boundary other) => this.Node == other.Node && this.Offset == other.Offset;
      ...
      public INode ChildAtOffset => this.Node.ChildNodes.Length <= this.Offset ? (INode) null : this.Node.ChildNodes[this.Offset];
    }
  }
}
