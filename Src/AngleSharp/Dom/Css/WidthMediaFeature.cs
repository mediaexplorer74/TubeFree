...
// Type: AngleSharp.Dom.Css.WidthMediaFeature
...
namespace AngleSharp.Dom.Css
{
  internal sealed class WidthMediaFeature : MediaFeature
  {
    ...
    public override bool Validate(RenderDevice device)
    {
      ...
      float viewPortWidth = (float) device.ViewPortWidth;
      if (this.IsMaximum)
        return (double) viewPortWidth <= (double) pixel;
      return this.IsMinimum ? (double) viewPortWidth >= (double) pixel : (double) pixel == (double) viewPortWidth;
    }
  }
}
