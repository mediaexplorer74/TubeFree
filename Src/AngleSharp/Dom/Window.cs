...
namespace AngleSharp.Dom
{
    ...
    IWindowTimers
  {
    ...
    public INavigator Navigator => this._navigator ?? (this._navigator = this._document.Context.CreateService<INavigator>());
    ...
    {
      IConfiguration options1 = this._document.Options;
      StyleOptions options2 = new StyleOptions(this._document.Context);
      return (IMediaQueryList) new CssMediaQueryList((IWindow) this, options1.GetCssStyleEngine().ParseMedia(mediaText, options2));
    }

    public ICssStyleDeclaration GetComputedStyle(IElement element, string pseudo = null) => (ICssStyleDeclaration) this.GetStyleCollection().ComputeDeclarations(element, pseudo);
    ...
    {
      HtmlDocument htmlDocument = new HtmlDocument(this._document.NewContext(name, Sandboxes.None));
    ...
    int IWindowTimers.SetTimeout(Action<IWindow> handler, int timeout) => this.QueueTask(new Func<Action<IWindow>, int, CancellationTokenSource, Task>(this.DoTimeoutAsync), handler, timeout);
    ...
    int IWindowTimers.SetInterval(Action<IWindow> handler, int timeout) => this.QueueTask(new Func<Action<IWindow>, int, CancellationTokenSource, Task>(this.DoIntervalAsync), handler, timeout);
