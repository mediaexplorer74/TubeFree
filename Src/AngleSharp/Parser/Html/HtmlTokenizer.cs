...
// Type: AngleSharp.Parser.Html.HtmlTokenizer
...
label_6:
    ...
    private HtmlToken Rawtext(char c) => c != '<' ? this.RawtextText(c) : this.RawtextLT(this.GetNext());
...
label_5:
    ...
    {
      if (c.IsSpaceCharacter() || c == '<' || c == char.MaxValue || c == '&' || (int) c == (int) allowedCharacter)
      {
        this.Back();
        this.StringBuffer.Append('&');
      }
...
label_11:
    ...
    private HtmlToken TagSelfClosing(HtmlTagToken tag) => this.TagSelfClosingInner(tag) ?? this.ParseAttributes(tag);
      ...
      while (true)
      {
        switch (attributeState)
        {
          ...
          case HtmlTokenizer.AttributeState.Name:
            ...
            {
              case '=':
                tag.AddAttribute(this.FlushBuffer());
...
label_83:
    ...
    private HtmlToken NewCharacter() => new HtmlToken(HtmlTokenType.Character, this._position, this.FlushBuffer());

    private HtmlToken NewComment() => new HtmlToken(HtmlTokenType.Comment, this._position, this.FlushBuffer());
    ...
    private HtmlDoctypeToken NewDoctype(bool quirksForced) => new HtmlDoctypeToken(quirksForced, this._position);
    ...
    private void RaiseErrorOccurred(HtmlParseError code) => this.RaiseErrorOccurred(code, this.GetCurrentPosition());
      ...
      switch (tag.Type)
      {
        case HtmlTokenType.StartTag:
          for (int index1 = attributes.Count - 1; index1 > 0; --index1)
          {
            ...
            {
              if (attributes[index2].Key == attributes[index1].Key)
              {
