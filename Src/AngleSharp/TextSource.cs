...
namespace AngleSharp
{
  public sealed class TextSource : IDisposable
  {
    ...
    public Task PrefetchAsync(int length, CancellationToken cancellationToken) => this.ExpandBufferAsync((long) length, cancellationToken);
    ...
    private async Task ReadIntoBufferAsync(CancellationToken cancellationToken) => this.AppendContentFromBuffer(await this._baseStream.ReadAsync(this._buffer, 0, 4096, cancellationToken).ConfigureAwait(false));
    ...
    private void ReadIntoBuffer() => this.AppendContentFromBuffer(this._baseStream.Read(this._buffer, 0, 4096));
