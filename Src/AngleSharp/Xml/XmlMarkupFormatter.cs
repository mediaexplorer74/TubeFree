...
// Type: AngleSharp.Xml.XmlMarkupFormatter
...
namespace AngleSharp.Xml
{
  ...
  {
    public static readonly IMarkupFormatter Instance = (IMarkupFormatter) new XmlMarkupFormatter();
    ...
    string IMarkupFormatter.OpenTag(IElement element, bool selfClosing)
    {
      ...
      foreach (IAttr attribute in (IEnumerable<IAttr>) element.Attributes)
        sb.Append(" ").Append(XmlMarkupFormatter.Instance.Attribute(attribute));
      if (selfClosing)
        sb.Append(" /");
      sb.Append('>');
      return sb.ToPool();
