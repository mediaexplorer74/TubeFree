...
// Type: AngleSharp.Parser.Html.HtmlDomBuilder
...
namespace AngleSharp.Parser.Html
{
  internal sealed class HtmlDomBuilder
  {
    ...
    public HtmlDomBuilder(HtmlDocument document)
    {
      ...
      this._tokenizer = new HtmlTokenizer(document.Source, resolver);
      this._tokenizer.Error += (EventHandler<HtmlErrorEvent>) ((_, error) => context.Fire((Event) error));
    ...
    public Element AdjustedCurrentNode => this._fragmentContext == null || this._openElements.Count != 1 ? this.CurrentNode : this._fragmentContext;

    public Element CurrentNode => this._openElements.Count <= 0 ? (Element) null : this._openElements[this._openElements.Count - 1];
    ...
    {
      this._fragmentContext = context != null ? context : throw new ArgumentNullException(nameof (context));
      ...
      this._document.AddNode((Node) htmlHtmlElement);
    ...
    private void Initial(HtmlToken token)
    {
      switch (token.Type)
      {
        case HtmlTokenType.Doctype:
          ...
          this._document.AddNode((Node) new DocumentType((Document) this._document, doctype.Name ?? string.Empty)
          {
      ...
      switch (token.Type)
      {
        ...
        case HtmlTokenType.StartTag:
          ...
          else
          {
            ...
            if (name1.IsOneOf(TagNames.Style, TagNames.NoFrames) || this._options.IsScripting && name1.Is(TagNames.NoScript))
            {
    ...
    private void InBodyStartTag(HtmlTagToken tag)
    {
      ...
      else if (name.Is(TagNames.IsIndex))
      {
        ...
        for (int index = 0; index < tag.Attributes.Count; ++index)
        {
          ...
          if (!attribute.Key.IsOneOf(AttributeNames.Name, AttributeNames.Action, AttributeNames.Prompt))
          {
            HtmlTagToken htmlTagToken = token;
            attribute = tag.Attributes[index];
            string key = attribute.Key;
            attribute = tag.Attributes[index];
            string str = attribute.Value;
            htmlTagToken.AddAttribute(key, str);
          }
        }
...
label_5:
        ...
        while (true)
        {
          ...
          if (openElement2 != element1)
          {
            ...
            else
            {
              ...
              openElement1.AddNode((Node) element4);
              ...
              element5.AddNode((Node) element3);
              element3 = element5;
            }
          }
          else
      ...
      switch (token.Type)
      {
        ...
        case HtmlTokenType.StartTag:
          ...
          if (name1.Is(TagNames.Font))
          {
            for (int index = 0; index != tag.Attributes.Count; ++index)
            {
              if (tag.Attributes[index].Key.IsOneOf(AttributeNames.Color, AttributeNames.Face, AttributeNames.Size))
              {
    ...
    private void ForeignSpecialTag(HtmlTagToken tag)
    {
      ...
      this.CurrentNode.AddNode((Node) foreignElementFrom);
      if (num != 0)
    ...
    private Element CreateForeignElementFrom(HtmlTagToken tag)
    {
      ...
      {
        MathElement element = this._mathFactory.Create((Document) this._document, tag.Name);
        this.AuxiliarySetupSteps((Element) element, tag);
        return (Element) element.Setup(tag);
      }
      if ((this.AdjustedCurrentNode.Flags & NodeFlags.SvgMember) != NodeFlags.SvgMember)
        return (Element) null;
      SvgElement element1 = this._svgFactory.Create((Document) this._document, tag.Name.SanatizeSvgTagName());
      this.AuxiliarySetupSteps((Element) element1, tag);
      return (Element) element1.Setup(tag);
    }
    ...
    {
      ConfiguredTaskAwaitable configuredTaskAwaitable = this._document.WaitForReadyAsync().ConfigureAwait(false);
      await configuredTaskAwaitable;
      configuredTaskAwaitable = script.RunAsync(CancellationToken.None).ConfigureAwait(false);
      await configuredTaskAwaitable;
    }
    ...
    {
      HtmlElement htmlElement = this._htmlFactory.Create((Document) this._document, tag.Name);
    ...
    private void AddElement(Element element)
    {
      ...
      if (this._foster && TagNames.AllTableMajor.Contains(currentNode.LocalName))
        ...
        currentNode.AddNode((Node) element);
    ...
    private void AddElementWithFoster(Element element)
    {
      ...
      while (--count != 0)
      {
        ...
        {
          this._openElements[count].AddNode((Node) element);
          return;
        }
      ...
      Node node = this._openElements[count].Parent ?? (Node) this._openElements[count + 1];
      if (flag && this._openElements[count].Parent != null)
      {
        for (int index = 0; index < node.ChildNodes.Length; ++index)
        {
          ...
          {
            node.InsertNode(index, (Node) element);
            break;
          }
        }
      }
    ...
    private void RaiseErrorOccurred(HtmlParseError code, HtmlToken token) => this._tokenizer.RaiseErrorOccurred(code, token.Position);
  }
}
