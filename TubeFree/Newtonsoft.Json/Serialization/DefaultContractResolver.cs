...
namespace Newtonsoft.Json.Serialization
{
  public class DefaultContractResolver : IContractResolver
  {
    ...
    private static readonly JsonConverter[] BuiltInConverters = new JsonConverter[6]
    {
    ...
    private bool ShouldSerializeEntityMember(MemberInfo memberInfo) => !(memberInfo is PropertyInfo propertyInfo) || !propertyInfo.PropertyType.IsGenericType() || !(propertyInfo.PropertyType.GetGenericTypeDefinition().FullName == "System.Data.Objects.DataClasses.EntityReference`1");
    ...
    private static void SetExtensionDataDelegates(JsonObjectContract contract, MemberInfo member)
    {
      ...
      if (attribute.ReadData)
      {
        ...
        PropertyInfo property1 = memberUnderlyingType.GetProperty("Item", BindingFlags.Instance | BindingFlags.Public, (object) null, genericArgument2, (IList<Type>) new Type[1]
        {
          genericArgument1
        }, (object) null);
        MethodInfo setMethod = property1 != null ? property1.GetSetMethod() : (MethodInfo) null;
        if (setMethod == null)
        {
          PropertyInfo property2 = implementingType.GetProperty("Item", BindingFlags.Instance | BindingFlags.Public, (object) null, genericArgument2, (IList<Type>) new Type[1]
          {
            genericArgument1
          }, (object) null);
          setMethod = property2 != null ? property2.GetSetMethod() : (MethodInfo) null;
        }
      ...
      {
        ObjectConstructor<object> createEnumerableWrapper = JsonTypeReflector.ReflectionDelegateFactory.CreateParameterizedConstructor((MethodBase) typeof (DefaultContractResolver.EnumerableDictionaryWrapper<,>).MakeGenericType(genericArgument1, genericArgument2).GetConstructors().First<ConstructorInfo>());
    ...
    {
      IEnumerator<ConstructorInfo> enumerator = objectType.GetConstructors(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic).Where<ConstructorInfo>((Func<ConstructorInfo, bool>) (c => c.IsDefined(typeof (JsonConstructorAttribute), true))).GetEnumerator();
      ...
      return objectType.GetConstructor((IList<Type>) new Type[4]
      {
        typeof (int),
        typeof (int),
        typeof (int),
      ...
      out List<SerializationErrorCallback> onError)
    {
      ...
      foreach (Type type1 in this.GetClassHierarchyForType(type))
      {
        ...
        bool flag1 = DefaultContractResolver.ShouldSkipSerializing(type1);
        bool flag2 = DefaultContractResolver.ShouldSkipDeserialized(type1);
    ...
    protected virtual JsonContract CreateContract(Type objectType)
    {
      ...
      switch (JsonTypeReflector.GetCachedAttribute<JsonContainerAttribute>((object) type))
      {
        ...
        default:
          ...
          return typeof (IDynamicMetaObjectProvider).IsAssignableFrom(type) ? (JsonContract) this.CreateDynamicContract(objectType) : (JsonContract) this.CreateObjectContract(objectType);
      }
    }
    ...
    internal static bool CanConvertToString(Type type) => type == typeof (Type) || type.IsSubclassOf(typeof (Type));
    ...
    protected virtual IValueProvider CreateMemberValueProvider(MemberInfo member) => (IValueProvider) new ExpressionValueProvider(member);
    ...
    {
      MethodInfo method = member.DeclaringType.GetMethod("ShouldSerialize" + member.Name, (IList<Type>) ReflectionUtils.EmptyTypes);
