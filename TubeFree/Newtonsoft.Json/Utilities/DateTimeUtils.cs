...
namespace Newtonsoft.Json.Utilities
{
  internal static class DateTimeUtils
  {
    ...
    private static long ToUniversalTicks(DateTime dateTime) => dateTime.Kind == DateTimeKind.Utc ? dateTime.Ticks : DateTimeUtils.ToUniversalTicks(dateTime, dateTime.GetUtcOffset());
    ...
    internal static long ConvertDateTimeToJavaScriptTicks(DateTime dateTime, TimeSpan offset) => DateTimeUtils.UniversialTicksToJavaScriptTicks(DateTimeUtils.ToUniversalTicks(dateTime, offset));

    internal static long ConvertDateTimeToJavaScriptTicks(DateTime dateTime) => DateTimeUtils.ConvertDateTimeToJavaScriptTicks(dateTime, true);

    internal static long ConvertDateTimeToJavaScriptTicks(DateTime dateTime, bool convertToUtc) => DateTimeUtils.UniversialTicksToJavaScriptTicks(convertToUtc ? DateTimeUtils.ToUniversalTicks(dateTime) : dateTime.Ticks);

    private static long UniversialTicksToJavaScriptTicks(long universialTicks) => (universialTicks - DateTimeUtils.InitialJavaScriptDateTicks) / 10000L;

    internal static DateTime ConvertJavaScriptTicksToDateTime(long javaScriptTicks) => new DateTime(javaScriptTicks * 10000L + DateTimeUtils.InitialJavaScriptDateTicks, DateTimeKind.Utc);
