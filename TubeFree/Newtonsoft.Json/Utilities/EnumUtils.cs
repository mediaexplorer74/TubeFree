...
namespace Newtonsoft.Json.Utilities
{
  internal static class EnumUtils
  {
    ...
    private static EnumInfo InitializeValuesAndNames(StructMultiKey<Type, NamingStrategy> key)
    {
      ...
      {
        string member = names[count];
        FieldInfo field = type.GetField(member, BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic);
        values[count] = EnumUtils.ToUInt64(field.GetValue((object) null));
        string name = field.GetCustomAttributes(typeof (EnumMemberAttribute), true).Cast<EnumMemberAttribute>().Select<EnumMemberAttribute, string>((Func<EnumMemberAttribute, string>) (a => a.Value)).SingleOrDefault<string>() ?? field.Name;
        if (Array.IndexOf<string>(strArray, name, 0, count) != -1)
          throw new InvalidOperationException("Enum name '{0}' already exists on enum '{1}'.".FormatWith((IFormatProvider) CultureInfo.InvariantCulture, (object) name, (object) type.Name));
        strArray[count] = key.Value2 != null ? key.Value2.GetPropertyName(name, false) : name;
      }
      return new EnumInfo(type.IsDefined(typeof (FlagsAttribute), false), values, names, strArray);
    }
    ...
    public static bool TryToString(Type enumType, object value, bool camelCase, out string name) => EnumUtils.TryToString(enumType, value, camelCase ? (NamingStrategy) EnumUtils._camelCaseNamingStrategy : (NamingStrategy) null, out name);
      ...
      bool disallowNumber)
    {
      ...
      if (indexByName.HasValue)
        return Enum.ToObject(enumType, (object) values[indexByName.Value]);
      int index1 = -1;
      ...
      return Enum.ToObject(enumType, (object) num1);
    }
