...
namespace Newtonsoft.Json.Utilities
{
  internal class ExpressionReflectionDelegateFactory : ReflectionDelegateFactory
  {
      ...
      ParameterExpression argsParameterExpression)
    {
      ...
      Expression expression2 = !(method is MethodInfo methodInfo) ? this.EnsureCastExpression(expression1, type) : (methodInfo.ReturnType == typeof (void) ? (Expression) Expression.Block(expression1, (Expression) Expression.Constant((object) null)) : this.EnsureCastExpression(expression1, type));
      ...
      bool allowWidening = false)
    {
      ...
      {
        MethodInfo method = typeof (Convert).GetMethod("To" + targetType.Name, (IList<Type>) new Type[1]
        {
          typeof (object)
        });
        if (method != null)
